...
// =======================
// IMAGE SERVING (for preview and PDF rendering)
// =======================

// Serve certificate images by path
router.get('/images/:filename', (req, res) => {
  const filename = req.params.filename;
  
  // Security: Only allow serving from certificates/images directory
  if (filename.includes('..') || filename.includes('/')) {
    return res.status(403).json({ error: 'Invalid filename' });
  }

  const imagePath = path.join(CERT_IMAGE_UPLOAD_DIR, filename);
